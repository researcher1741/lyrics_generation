...
from src.lyrics_datasets.pinyin_chinese import Pinyin, rhyme_zh
...
def rhyming_evaluation(generations, language=None):
    ...
    for item in tqdm(generations, desc='evaluating'):
        if 'language' in item and (language is None or language == 'multi'):
            lang = item['language']
        else:
            lang = language
        # if language is not None and 'language' in item and item['language'] != language:
        #   continue
        phonemizer = backends_dict[language2code[lang]]
        ...
        for i, l1 in enumerate(schema):
            for j in range(i + 1, len(schema)):
                l2 = schema[j]
                if l1 == l2:
                    if do_rhyme(sentences[i], sentences[j], phonemizer, lang):
                        correct += 1
                        local_correct += 1
                    tot += 1
                    local_tot += 1
        correct_for_macro.append(local_correct / max(local_tot, 1))
        local_fp, local_tot, local_fp_rate = compute_fp_rate(sentences, schema, lang, phonemizer)
        fp += local_fp
        fp_tot += local_tot
    ...
    return {'micro_precision': micro_precision, 'macro_precision': macro_precision,
...
def do_rhyme(sentence_1, sentence_2, phonemizer, language):
    ...
    if word_1.lower() == word_2.lower():
        return True
    if language == 'chinese':
        special_colon = b'\xef\xbc\x9a'.decode('utf8')
        if word_1 == '' or word_2 == '':
            return False
        elif word_1 == special_colon or word_2 == special_colon or word_1 == ':' or word_2 == ':':
            return False
        else:
            return rhyme_zh(word_1, word_2)
    else:
