...
def run_generation(conf, pl_module, pl_data_module, tokenizer, language = 'english', **gen_args):
    ...
    generation_args = dict(
                ...
                decoder_start_token_id=model_conf.decoder_start_token_id, #encode(RHYME_TOKENS[0], add_special_tokens=False)[0],
    ...
    with torch.no_grad():
        ...
        jsonlines.open(gen_path + '.jsonl', 'w') as gen_json_writer, jsonlines.open(gold_path + '.jsonl', 'w') as gold_json_writer: 
            ...
            for batch in tqdm(test_loader, desc='generating'):
                ...
                if 'decoder_input_ids' in batch and batch['decoder_input_ids'][0][0].item() != generation_args['decoder_start_token_id']:
                    print("WARN: detected wrong decoder_start_token_id, changing it from ",  generation_args['decoder_start_token_id'], 
                        f'({tokenizer.decode(generation_args["decoder_start_token_id"])}) to', batch['decoder_input_ids'][0][0].item(), 
...
def test(conf: omegaconf.DictConfig) -> None:
    ...
    _, pl_module, pl_data_module, tokenizer = init_everything_for_evaluation(conf)
...
def get_rhyming_word_and_rhyme_letter(sentence, use_sep=False):
    ...
    rhyme_letter = re.findall(r'RHYME_[A-Z]', sentence)[0]
    if use_sep:
        if SEP in sentence:
            return sentence[7:sentence.index(SEP)].strip(), rhyme_letter
        else: return '', rhyme_letter
...
def dump_for_evaluation(generation_writer, gold_writer, 
    ...
    decoded_gens = [dg.replace('[CLS]', '').strip() for dg in decoded_gens]
    ...
    if gold_ids is not None:
        decoded_golds = [tokenizer.decode(g).replace(END_LYRICS, '').replace(tokenizer.pad_token, '').strip() for g in gold_ids]
    else:
    ...
    for inp, gen, gold, schema, lang in zip(decoded_inp, top_gens, decoded_golds, top_schemas, elem_languages):
        gen = gen.replace(inp, '').replace(tokenizer.pad_token, '').replace('<pad>', '').replace(SENTENCE_END, SENTENCE_END + '\n').replace(TAG_END, TAG_END + '\n')
        gen = gen.replace(BLOCK_END, '\n' + BLOCK_END + '\n')
        inp = inp.replace(tokenizer.pad_token, '').replace('<pad>', '').replace(SENTENCE_END, SENTENCE_END + '\n').replace(TAG_END, TAG_END + '\n')
        if gold :
            gold = gold.replace(tokenizer.pad_token, '').replace('<pad>', '').replace(SENTENCE_END, SENTENCE_END + '\n').replace(TAG_END, TAG_END + '\n')
        ...
        generation_json_writer.write({'generation':gen, 'input': inp, 'gold': gold, 'rhyming_schema': schema, 'language': lang})
