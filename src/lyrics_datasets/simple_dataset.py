...
class SimpleDataset():
    ...
    def _load_examples(self, path):
        examples = list()
        batch_size = 512
        keys_batch, input_batch, output_batch, schema_batch = [], [], [], []
        with jsonlines.open(path) as lines:
            for line in tqdm(lines, desc='loading ' + self.split_name):
                ...
                if len(input_batch) == batch_size:
                    ...
                    for k, ids, l, s in zip(keys_batch, input_ids, labels, schema_batch):
                        examples.append({'key': k, 'input_ids': ids, 'labels': l, 'schema': s})
                    input_batch, output_batch, keys_batch, schema_batch = [], [], [], []
