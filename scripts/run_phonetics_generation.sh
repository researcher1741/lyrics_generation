...
if [ "$#" -ne 2 ]; then
    echo 'Usage:'
    echo './run_phonetics_generation.sh /path/to/checkpoint /path/to/tokenizer/'
    exit 1
fi
if [[ "$1" = /* ]]; then
    CKPT_PATH=$1
...
PYTHONPATH=src python src/lyrics_generation/generate_phonetics.py +checkpoint_path=\"$CKPT_PATH\"\
    +out_dir=\"$OUT_DIR\"\
